...
```{r}
estimated_variance <- mean((knn_preds - mean(knn_preds))^2)
estimated_variance
```
...
prediction_error
```
The prediction error is 2.424368.
...
```
The prediction error is 2.424368, which is very close to the theoretical calculation 2.586353 in the previous question, which implies that the theoretical model is a good approximation of the actual behavior of the KNN model in practice.
...
```
If our goal is general accuracy and balanced performance (with a trade-off between false positives and false negatives), the cutoff = 0.5 is preferred, as it has the lowest test error and higher precision compared to the other cutoffs.

If Our Goal is to Capture as Many People Who Will Default as Possible
...
# Prepare the data
train_X <- as.matrix(mnist[1:1250, -1])  # Pixels only for training data
train_Y <- as.factor(mnist[1:1250, 1])   # Digits for training labels as factor
test_X <- as.matrix(mnist[1251:2500, -1]) # Pixels only for test data
test_Y <- as.factor(mnist[1251:2500, 1])  # Digits for test labels as factor

# Perform cross-validation to find lambda_1se
cv_fit <- cv.glmnet(train_X, train_Y, family = "multinomial", alpha = 1, nfolds = 20)

# Get the lambda value at 1 standard error from the minimum (lambda_1se)
lambda_1se <- cv_fit$lambda.1se
print(lambda_1se)

# Fit the final model with lambda_1se
final_model <- glmnet(train_X, train_Y, family = "multinomial", alpha = 1, lambda = lambda_1se)

# Predict the class labels on the test set
test_predictions <- predict(final_model, newx = test_X, s = lambda_1se, type = "class")

# Generate the confusion matrix
confusion_mat_log <- table(Predicted = test_predictions, Actual = test_Y)
print(confusion_mat_log)

# Calculate the test error
test_accuracy <- sum(diag(confusion_mat_log)) / sum(confusion_mat_log)

...
$$\bX = \bU \bD \bV^\T$$
The $\bV$ matrix here corresponds to the directions we found. Hence, $\bv_1$ is its first column, $\bv_1$ is its second column, etc.. $\bD$ is a diagonal matrix ordered from the largest to the smallest values, correspond to the standard deviation of the spreads. And $\bU$ represents the coordinates once we project $\bX$ onto those directions. 

## Optimization View

An alternative way to understand this is by matrix approximation, if we want to find a rank-1 matrix that best approximate $\bX$ with the Frobenius norm, we optimize 
